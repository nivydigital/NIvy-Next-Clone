...
from backend.app.runtime.workflows.lead_outreach import (  # noqa: E402
...
def test_idempotency_key_stable():
    ...
    k2 = make_idempotency_key(
        "camp-1", {"email": "a@b.com", "company": "acme", "external_id": "x1"}
    )
    assert k1 == k2
...
def test_state_machine_happy_path_dry_run():
    ...
    assert "draft" in run.steps_completed
    assert run.artifacts.get("email_draft", {}).get("to") == "prospect@acme.test"
    ...
    receipt = run.artifacts["send_receipt"]
    assert receipt.get("dry_run") is True or receipt.get("mock") is True
    ...
    evidence = {
        "run": run.to_dict(),
        "notes": "Controlled E2E with mocks; no external send",
    }
    (EVIDENCE_DIR / "happy_path_dry_run.json").write_text(json.dumps(evidence, indent=2))
    assert (EVIDENCE_DIR / "happy_path_dry_run.json").exists()


def test_reject_cancels():
    orch = LeadOutreachOrchestrator()
    run = orch.start(
        campaign_id="c-reject",
        lead={"email": "r@x.com", "company": "X", "name": "R"},
        dry_run=True,
    )
...
def test_send_requires_approval_state():
    orch = LeadOutreachOrchestrator()
    run = orch.start(
        campaign_id="c-no-approve",
        lead={"email": "n@x.com", "company": "X", "name": "N"},
        dry_run=True,
    )
    try:
...
def test_illegal_transition():
    orch = LeadOutreachOrchestrator()
    run = orch.start(
        campaign_id="c-illegal",
        lead={"email": "i@x.com", "company": "X", "name": "I"},
        dry_run=True,
    )
    try:
