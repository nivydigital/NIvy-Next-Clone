"""Knowledge ingest: extract → chunk → embed → store (Phase 7). Mock when infra unset."""
...
class Chunk:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"chunk_id": self.chunk_id, "pack_id": self.pack_id, "section": self.section, "text": self.text, "token_estimate": self.token_estimate, "metadata": self.metadata}
...
class IngestResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"pack_id": self.pack_id, "chunk_count": len(self.chunks), "chunks": [c.to_dict() for c in self.chunks], "stored": self.stored, "mock": self.mock, "ingested_at": self.ingested_at}
...
def chunk_markdown(pack_id: str, text: str, max_chars: int = 800) -> list[Chunk]:
    ...
    for section, body in sections:
        ...
        for i in range(0, len(body), max_chars):
            ...
            chunks.append(Chunk(chunk_id=f"{pack_id}-{digest}", pack_id=pack_id, section=section, text=part, token_estimate=max(1, len(part) // 4), metadata={"pack_id": pack_id, "section": section, "source_path": f"knowledge/packs/{pack_id}.md", "part_index": i}))
    return chunks


def embed_texts(texts: list[str]) -> list[list[float]]:
    return [[((b / 255.0) * 2 - 1) for b in hashlib.sha256(t.encode()).digest()[:8]] for t in texts]
...
class InMemoryStore:
    ...
    def upsert_vector(self, chunk: Chunk, vector: list[float]) -> None:
        self.vectors[chunk.chunk_id] = {"chunk_id": chunk.chunk_id, "pack_id": chunk.pack_id, "section": chunk.section, "vector": vector, "text": chunk.text, "metadata": chunk.metadata}
...
def ingest_pack(pack_id: str, *, actor: str = "system") -> IngestResult:
    ...
    for chunk, vector in zip(chunks, vectors, strict=True):
        ...
        stored["qdrant"] += 1
        store.insert_row({"chunk_id": chunk.chunk_id, "pack_id": pack_id, "section": chunk.section, "object_key": obj_key, "token_estimate": chunk.token_estimate, "ingested_at": _now(), "provenance": chunk.metadata})
        stored["postgres"] += 1
    result = IngestResult(pack_id=pack_id, chunks=chunks, stored=stored, mock=mock)
    audit_log.record(event_type="knowledge.ingest", actor=actor, action=pack_id, status="completed", request_id=str(uuid4()), data={"chunk_count": len(chunks), "mock": mock})
    return result
